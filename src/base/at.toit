...
class Command:
  ...
  write-parameters buffer/io.Writer:
    first := true
    parameters.do:
  ...
  while index < size:
    ...
    if end <= index:
      ...
      if (line.index-of '.' --from=index --to=end) >= 0:
        ...
        i := int.parse line[index..end] --radix=10 --if-error=: null
        parts.add
...
class Session:
  ...
  */
  ...
  write-command_ command/Command:
    logger_.with-level log.DEBUG-LEVEL: it.debug "-> $command"
  ...
  dispatch-urc_ urc/string response/List:
    ...
    urc-handlers_.do: | prefix lambda |
      if urc.starts-with prefix:
        logger_.with-level log.DEBUG-LEVEL: it.debug "<- [URC] $urc $response"
      ...
      if c == '+':
        ...
        if command := command_:
          if data := command.data:
            ...
            writer_.write data
            logger_.with-level log.DEBUG-LEVEL: it.debug "<- $(%c data-marker)"
            logger_.with-level log.DEBUG-LEVEL: it.debug "-> <$(data.size) bytes>"
            continue
  ...
  read-formatted_:
    ...
    if is-terminating_ cmd-bytes:
      ...
      logger_.with-level log.DEBUG-LEVEL: it.debug "<- $line"
    ...
    parsed := response-parsers_.get cmd
      --if-present=:
        logger_.with-level log.DEBUG-LEVEL: it.debug "<- $cmd: <custom>"
        it.call reader_
      --if-absent=:
        bytes := reader_.read-bytes line-end - cmd-end
        reader_.skip 1  // Skip s3.
        logger_.with-level log.DEBUG-LEVEL: it.debug "<- $cmd: $bytes.to-string-non-throwing"
  ...
  read-plain_:
    ...
    logger_.with-level log.DEBUG-LEVEL: it.debug "<- $line.to-string-non-throwing"
